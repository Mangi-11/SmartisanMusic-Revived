...
前景资源将 `18dp / 108dp` 编码为 `16.6667%` 比例内缩，而不是固定 `18dp`。真机上的 ColorOS Toast 以 `20dp` 显示应用图标；固定尺寸内缩会大于 `AdaptiveIconDrawable` 分配给前景层的边界，使唱片前景完全消失、只剩黄色背景。比例内缩在桌面、应用详情和小尺寸系统界面中保持同一几何关系，无需为 Toast 增加静态应用图标或自定义提示层。
...
- 触发播放页“屏幕常亮”等系统 Toast，确认小尺寸图标同时显示黄色背景和唱片前景。
