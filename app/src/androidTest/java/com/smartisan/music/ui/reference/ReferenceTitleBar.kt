// Frozen pre-migration title renderer. Used only for pixel and hit-bound regression tests.

package com.smartisan.music.ui.reference
...
constructor(
        ...
        titleView =
            TextView(context).apply {
                ...
                setTextSize(
                    ...
                    resources.getDimension(R.dimen.title_text_size),
                )
    ...
    fun getLeftViewByIndex(index: Int): View? =
        leftViews.filter { it.visibility != View.GONE }.getOrNull(index)

    fun getRightViewByIndex(index: Int): View? =
        rightViews.filter { it.visibility != View.GONE }.getOrNull(index)
        ...
        sideViews.forEachIndexed { index, view ->
            ...
            if (alignLeft) {
                if (visibleIndex == 0) {
                    ...
                    val previous = sideViews.take(index).lastOrNull { it.visibility != View.GONE }
                    if (previous != null) {
    ...
    private class TitleBarIconScaleTouchListener(view: View) : View.OnTouchListener {
        ...
        override fun onTouch(view: View, event: MotionEvent): Boolean {
            when (event.actionMasked) {
                ...
                MotionEvent.ACTION_MOVE ->
                    playScale(if (view.isPressed) TitleBarIconPressedScale else 1f)
                MotionEvent.ACTION_UP,
                MotionEvent.ACTION_CANCEL -> playScale(1f)
            }
            return false
        }
